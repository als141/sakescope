...
function buildToolManifest(): ToolManifest[] {
  const agent = createEmbedVoiceAgent();
...
function buildInstructions(): string {
  ...
  return typeof agent.instructions === 'string'
    ? agent.instructions
    : 'You are a helpful sake sommelier (embed).';
}
...
export async function POST() {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error('OpenAI API error (embed):', errorData);
      return NextResponse.json(
    ...
    console.error('Error creating client secret (embed):', error);
    return NextResponse.json(
