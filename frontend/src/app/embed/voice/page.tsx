...
export default function VoiceOnlyEmbedPage() {
  ...
  const mainSpacingClass = isVoiceSessionMobile
    ? 'pt-0 pb-0 gap-0 sm:pt-14 sm:pb-20 sm:gap-10'
    : isVoiceConnected
      ? 'pt-14 pb-24 gap-8 sm:gap-12'
      : 'pt-20 pb-28 gap-10 sm:gap-14';
      ...
      >
        ...
        <AnimatePresence mode="wait">
          {recommendedSake ? (
            ...
            <motion.div
              key="hero"
              className="text-center space-y-6 sm:space-y-8 w-full max-w-3xl px-3 sm:px-6 mt-8 sm:mt-4 flex flex-col items-center"
              initial={{ opacity: 0, scale: 0.96, y: 14 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.96, y: -14 }}
              transition={{ duration: 0.4 }}
            >
              <h2 className="text-3xl sm:text-5xl md:text-6xl font-bold leading-[1.08] tracking-tight">
